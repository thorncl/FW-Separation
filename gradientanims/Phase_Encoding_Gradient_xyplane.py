...
def update(frame):
    ...
    phase = 180 * (Y + 5) / 10  
    U, V = U * np.cos(np.radians(phase)) - V * np.sin(np.radians(phase)), U * np.sin(np.radians(phase)) + V * np.cos(np.radians(phase))
...
ax.set_xticks([])
ax.set_yticks([])
ax.set_title('Frequency and Phase Encoding')
plt.show()
ani.save('phase_encoding_xyplane.gif', writer='pillow', fps=20)
HTML(ani.to_jshtml())