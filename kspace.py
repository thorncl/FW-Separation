...
class kspace_data():
    ...
    def __init__(self, kx_params, ky_params):
        ...
        self.params = {"k_xy_params": [self.kx_params, self.ky_params], "k_xy": [self.ky, self.kx]}
    ...
    def shift_term(self, data: np.ndarray, ax: int) -> np.ndarray:
        ...
        return np.where(data % param.row == 0, param.shift, -param.shift) if param.shift != 0 else 0
